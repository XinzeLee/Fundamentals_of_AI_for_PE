...
[`ridge_polynomial_regression.ipynb`](ridge_polynomial_regression.ipynb) — **Section III-C – III-E**. 
[`classic_ML.ipynb`](classic_ML.ipynb) — **Section III-B**, **III-E**. 
[`gaussian_process_bayesian_optimization.ipynb`](gaussian_process_bayesian_optimization.ipynb) — **Section III-E**, **V-C**.
...
>
> In practice, EDA, preprocessing, training, and tuning often form an **iterative loop**: prediction errors or abnormal results can reveal data issues and motivate further refinement of acquisition, preprocessing, or model choice—themes covered in **Section III-B – III-E** and in notebooks such as [`classic_ML.ipynb`](classic_ML.ipynb) and [`ridge_polynomial_regression.ipynb`](ridge_polynomial_regression.ipynb).
>
> <p align="center">
...
>
> Hyperparameter search for surrogate models also connects to **Section V-C**, as in [`gaussian_process_bayesian_optimization.ipynb`](gaussian_process_bayesian_optimization.ipynb).
...
- `gaussian_process_bayesian_optimization.ipynb` — GP regression on `fetch_california_housing`; expected-improvement Bayesian Optimization for kernel/noise hyperparameters; diagnostic plots  
...
- **Gaussian process regression** with predictive uncertainty; **Bayesian optimization** to tune GP hyperparameters on a validation split (`gaussian_process_bayesian_optimization.ipynb`)  
...
- Fuzzy decision tree to improve regression surface contours
...
- **`gaussian_process_bayesian_optimization.ipynb`** — GP regression on California housing + expected-improvement Bayesian Optimization for kernel/noise hyperparameters.  
- **`classic_ML.ipynb`** — Breast Cancer classification; decision boundaries (optional fuzzy tree branch).  

## Algorithms & data

- **Algorithms:** `DecisionTreeClassifier`; `FuzzyDecisionTreeClassifier`; in the GP notebook — `GaussianProcessRegressor` (constant × RBF), custom expected-improvement loop.  
- **Data:** Breast Cancer Wisconsin (built-in); California housing (built-in, GP notebook).
...
2. `gaussian_process_bayesian_optimization.ipynb` for surrogate modeling and Bayesian Optimization (optional second regression track).  
